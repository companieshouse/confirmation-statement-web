...
const URL = TASK_LIST_PATH.replace(`:${urlParams.PARAM_COMPANY_NUMBER}`, COMPANY_NUMBER)
...
describe("Task list controller tests", () => {
    ...
    describe("get tests", () => {
        it("Should navigate to the task list page", async () => {
            ...
            expect(mockInitTaskList).toBeCalledWith(
            ...
            expect(response.text).toContain(
                "Check and confirm that the company information we have on record is correct"
            );
        ...
        it("Should show recordDate as sysdate when filing before the nextMadeUpToDate", async () => {
            if (validCompanyProfile.confirmationStatement === undefined) {
                ...
                const companyProfile = clone(validCompanyProfile);
                companyProfile.confirmationStatement.nextMadeUpTo =
                    DateTime.fromISO("2999-06-04T00:00:00.000Z").toString();
