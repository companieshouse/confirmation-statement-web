...
describe("Confirmation controller tests", () => {
    ...
    it("should show limited partnership confirmation page", async () => {
        ...
        const DIFFERENT_LIMITED_PARTNERSHIP_LINK_URL =
            "/company-lookup/search?forward=/confirmation-statement/confirm-company?companyNumber=%7BcompanyNumber%7D&amp;backLink=/confirmation-statement/";
        const LIMITED_PARTNERSHIP_OVERVIEW_LINK_URL =
            "/confirmation-statement/company/12345678/transaction/66454/submission/435435/acsp/confirmation?overview=true";
    ...
    it("Should navigate to error page if error is thrown when getting confirmation page", async () => {
        ...
        spyGetTrans.mockImplementationOnce(() => {
            throw new Error();
        });
        const response = await request(app).get(URL);
