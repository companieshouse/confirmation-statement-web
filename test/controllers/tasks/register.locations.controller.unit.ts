...
import {
    mockRegisterLocation,
    mockRegisterLocationNoReg,
    mockRegisterLocationNoRegNoSail,
} from "../../mocks/register.location.mock";
...
const OTHER_RECORDS_MESSAGE =
    "Any other company records are kept at the registered office address, or on the public record.";
...
const WRONG_REGISTER_LOCATIONS_URL = WRONG_REGISTER_LOCATIONS_PATH.replace(
...
describe("Register locations controller tests", () => {
    ...
    it("Should navigate to the task list page when register locations are confirmed", async () => {
        const response = await request(app).post(REGISTER_LOCATIONS_URL).send({ registers: RADIO_BUTTON_VALUE.YES });
    ...
    it("Should navigate to the register locations stop page if details are incorrect", async () => {
        const response = await request(app).post(REGISTER_LOCATIONS_URL).send({ registers: RADIO_BUTTON_VALUE.NO });
        expect(mockSendUpdate.mock.calls[0][1]).toBe(SECTIONS.REGISTER_LOCATIONS);
    ...
    it("Should return error page when radio button id is not valid", async () => {
        const response = await request(app).post(REGISTER_LOCATIONS_URL).send({ registers: "malicious code block" });
