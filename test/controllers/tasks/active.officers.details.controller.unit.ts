...
} from "../../mocks/active.officers.details.mock";
...
const ACTIVE_OFFICER_DETAILS_URL = ACTIVE_OFFICERS_DETAILS_PATH.replace(
...
describe("Active officers details controller tests", () => {
    ...
    describe("get tests", () => {
        ...
        it("Should display corporate secretary details", async () => {
            ...
            expect(response.text).toContain(
                "10 Corpsecret Road, Corpsecrettown, Corpsecretshire, Corpsecretland, CS1 7SC"
            );
        ...
        it("Should navigate to an error page if the called service throws an error", async () => {
            mockGetActiveOfficerDetails.mockImplementationOnce(() => {
                throw new Error();
            });
        ...
        it("Should not call identification type lookup when identification type is null", async () => {
            mockGetActiveOfficerDetails.mockReturnValueOnce([mockCorporateOfficerWithNullIdentificationType]);
    ...
    describe("post tests", () => {
        it("Should return to task list page when officer details are confirmed", async () => {
            const response = await request(app).post(ACTIVE_OFFICER_DETAILS_URL).send({ activeOfficers: "yes" });
        ...
        it("Should go to stop page when officer details radio button is no", async () => {
            const response = await request(app).post(ACTIVE_OFFICER_DETAILS_URL).send({ activeOfficers: "no" });
        ...
        it("Should return an error page if error is thrown in post function", async () => {
            ...
            spyGetUrl.mockImplementationOnce(() => {
                throw new Error();
            });
            const response = await request(app).post(ACTIVE_OFFICER_DETAILS_URL);
