...
const SHARES_TOTALS_INVALID_WARNING =
    "The company's share capital does not match the number of shares held by its shareholders.";
const UNPAID_AMOUNT_NULL_WARNING =
    "The total amount unpaid for all shares is missing on this company’s statement of capital.";
const COMPANY_NUMBER = "12345678";
const SUBMISSION_ID = "a80f09e2";
const TRANSACTION_ID = "111-111-111";
const STATEMENT_OF_CAPITAL_URL = urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
    STATEMENT_OF_CAPITAL_PATH,
    COMPANY_NUMBER,
    TRANSACTION_ID,
    SUBMISSION_ID
);
const TASK_LIST_URL = urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
    TASK_LIST_PATH,
    COMPANY_NUMBER,
    TRANSACTION_ID,
    SUBMISSION_ID
);
const WRONG_STATEMENT_OF_CAPITAL_URL = urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
    WRONG_STATEMENT_OF_CAPITAL_PATH,
    COMPANY_NUMBER,
    TRANSACTION_ID,
    SUBMISSION_ID
...
describe("Statement of Capital controller tests", () => {
    ...
    describe("get tests", () => {
        ...
        it("Should return an error page if error is thrown in get function", async () => {
            ...
            spyGetUrlWithCompanyNumber.mockImplementationOnce(() => {
                throw new Error();
            });
            const response = await request(app).get(STATEMENT_OF_CAPITAL_URL);
        ...
        it("Should return an error page if error is thrown when service is called", async () => {
            mockGetStatementOfCapitalData.mockImplementationOnce(() => {
                throw new Error();
            });
            const response = await request(app).get(STATEMENT_OF_CAPITAL_URL);
            expect(response.text).toContain(EXPECTED_ERROR_TEXT);
    ...
    describe("post tests", () => {
        ...
        it("Should navigate to the statement of capital stop page when statement of capital is declared incorrect", async () => {
            mockGetStatementOfCapitalData.mockResolvedValueOnce(mockStatementOfCapital);
            mockGetConfirmationStatement.mockResolvedValueOnce(mockConfirmationStatementSubmission);
            const response = await request(app).post(STATEMENT_OF_CAPITAL_URL).send({ statementOfCapital: "no" });
        ...
        it("Should return an error page if error is thrown in post function", async () => {
            const spyGetUrlWithCompanyNumberTransactionIdAndSubmissionId = jest.spyOn(
                urlUtils,
                "getUrlWithCompanyNumberTransactionIdAndSubmissionId"
            );
            spyGetUrlWithCompanyNumberTransactionIdAndSubmissionId.mockImplementationOnce(() => {
                throw new Error();
            });
