...
const PEOPLE_WITH_SIGNIFICANT_CONTROL_URL = urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
    PEOPLE_WITH_SIGNIFICANT_CONTROL_PATH,
    COMPANY_NUMBER,
    TRANSACTION_ID,
    SUBMISSION_ID
    ...
    describe("post tests", function () {
        ...
        it("should display error page if no psc is found when radio button is not selected", async () => {
            ...
            expect(mockCreateAndLogError).toHaveBeenCalledWith(
                expect.stringContaining("No PSC data found, no radio button selected")
            );
