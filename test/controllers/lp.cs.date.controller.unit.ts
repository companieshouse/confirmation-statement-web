...
const URL = LP_CS_DATE_PATH.replace(`:${urlParams.PARAM_COMPANY_NUMBER}`, COMPANY_NUMBER)
    .replace(`:${urlParams.PARAM_TRANSACTION_ID}`, TRANSACTION_ID)
    .replace(`:${urlParams.PARAM_SUBMISSION_ID}`, SUBMISSION_ID);
const PAGE_TITLE_ERROR = "Error: Confirmation statement date";
jest.mock("../../src/utils/properties", () => ({
    ...(jest.requireActual("../../src/utils/properties") as any),
    FEATURE_FLAG_LP_REFORM_DATE: "1970-01-01",
}));
...
describe("start confirmation statement date controller tests", () => {
    ...
    it("should forward to sic code page", async () => {
        ...
        expect(response.headers.location).toBe(
            "/confirmation-statement/company/12345678/transaction/66454/submission/435435/acsp/sic-code-summary?lang=en"
        );
        expect(mockResetReviewCheckboxes).not.toHaveBeenCalled();
    });
...
describe("date controller post tests", () => {
    beforeEach(() => {
        ...
        (limitedPartnershipUtils.getReviewPath as jest.Mock).mockReturnValue(
            "/confirmation-statement/company/12345678/transaction/66454/submission/435435/acsp/review"
        );
    });

    it("should redirect to SIC Summary page when 'no' is selected and company type is not pflp/spflp", async () => {
        const response = await request(app).post(URL).send({ confirmationStatementDate: "no" });

        expect(response.status).toBe(302);
        expect(response.headers.location).toBe(
            `/confirmation-statement/company/${COMPANY_NUMBER}/transaction/${TRANSACTION_ID}/submission/${SUBMISSION_ID}/acsp/sic-code-summary?lang=en`
        );
        expect(mockResetReviewCheckboxes).not.toHaveBeenCalled();
    ...
    it("should redirect to Review page when 'no' is selected and company type is pflp", async () => {
        ...
        expect(response.status).toBe(302);
        expect(response.headers.location).toBe(
            `/confirmation-statement/company/${COMPANY_NUMBER}/transaction/${TRANSACTION_ID}/submission/${SUBMISSION_ID}/acsp/review?lang=en`
        );
