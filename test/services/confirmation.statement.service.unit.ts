...
const mockPostNewConfirmationStatement = ConfirmationStatementService.prototype
    .postNewConfirmationStatement as jest.Mock;
const mockPostUpdateConfirmationStatement = ConfirmationStatementService.prototype
    .postUpdateConfirmationStatement as jest.Mock;
const mockCreateApiClient = createApiClient as jest.Mock;
const mockGetConfirmationStatementSubmission = ConfirmationStatementService.prototype
    .getConfirmationStatementSubmission as jest.Mock;
...
describe("Confirmation statement api service unit tests", () => {
    ...
    describe("createConfirmationStatement unit tests", () => {
        ...
        it("should throw error when failed post call", async () => {
            ...
            await createConfirmationStatement(getSessionRequest({ access_token: "token" }), TRANSACTION_ID)
                .then(() => {
    ...
    describe("getConfirmationStatement unit tests", () => {
        ...
        it("should throw error when http status is not 200", async () => {
            ...
            await getConfirmationStatement(getSessionRequest({ access_token: "token" }), TRANSACTION_ID, SUBMISSION_ID)
                .then(() => {
                    fail("Expecting error to be thrown");
                })
                .catch(e => {
        ...
        it("should throw error with any special characters encoded when http status is not 200", async () => {
            ...
            await getConfirmationStatement(
                getSessionRequest({ access_token: "token" }),
                TRANSACTION_ID_WITH_SPECIAL_CHARACTERS,
                SUBMISSION_ID_WITH_SPECIAL_CHARACTERS
            )
                .then(() => {
                    fail("Expecting error to be thrown");
                })
...
describe("updateConfirmationStatement unit tests", () => {
    ...
    it("should should throw error with any special characters encoded when other http code is returned", async () => {
        ...
        expect(mockPostUpdateConfirmationStatement).toBeCalledWith(
            TRANSACTION_ID_WITH_SPECIAL_CHARACTERS,
            SUBMISSION_ID_WITH_SPECIAL_CHARACTERS,
            csSubmission
        );
...
describe("getNextMadeUpToDate tests", () => {
    ...
    it("Should call SDK to get next made up to date", async () => {
        ...
        const nextMadeUpToDate: NextMadeUpToDate = await getNextMadeUpToDate(
            getSessionRequest({ access_token: "token" }),
            COMPANY_NUMBER
        );
    ...
    it("Should throw an error if status code != 200", async () => {
        ...
        await expect(getNextMadeUpToDate(getSessionRequest({ access_token: "token" }), COMPANY_NUMBER)).rejects.toThrow(
            ERROR
        );

        expect(mockCreateAndLogError).toBeCalledWith(
            expect.stringContaining(
                `Error getting next made up to date from api with company number = ${COMPANY_NUMBER}`
            )
        );
    });
    ...
    it("Should throw an error if no resource returned when status 200", async () => {
        ...
        expect(mockCreateAndLogError).toBeCalledWith(
            expect.stringContaining(
                `Error No resource returned when getting next made up to date from api with companyNumber = ${COMPANY_NUMBER}`
            )
        );
    });
});
