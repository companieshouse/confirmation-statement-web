...
describe("url utils tests", () => {
    ...
    describe("getUrlToPath tests", () => {
        it("Should populate the Url with the company number, transaction ID and submission ID from the request params", () => {
            ...
            expect(populatedUrl).toEqual(
                `/company/${COMPANY_NUMBER}/something/transaction/${TX_ID}/submission/${SUB_ID}/andThenSome`
            );
        });
    });
    ...
    describe("getUrlWithCompanyNumberTransactionIdAndSubmissionId tests", () => {
        it("Should the Url with the company number, transaction ID and submission ID", () => {
            const populatedUrl = urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
                urlWithParams,
                COMPANY_NUMBER,
                TX_ID,
                SUB_ID
    ...
    describe("sanitiseReqlUrls tests", () => {
        it("Should truncate the request urls", () => {
            const tooLongUrl =
                "http://something/something/something/12344/somethingelse/andAnotherThing/12345/somethingElse/something/something";
            req.url = tooLongUrl;
            req.originalUrl = tooLongUrl;
            urlUtils.sanitiseReqUrls(req);
            expect(req.url).toEqual(
                "http://something/something/something/12344/somethingelse/andAnotherThing/12345/somethingElse/somethi..."
            );
        ...
        it("Should truncate url params if they are too long", () => {
            ...
            expect(req.url).toEqual(
                "/company/5858488384.../something/transaction/3284338246.../submission/9398432847.../andThenSome"
            );
            expect(req.originalUrl).toEqual(
                "/company/5858488384.../something/transaction/3284338246.../submission/9398432847.../andThenSome"
            );
