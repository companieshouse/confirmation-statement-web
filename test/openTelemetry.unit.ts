...
describe("OpenTelemetry setup and shutdown", () => {
    ...
    it("starts OpenTelemetry when enabled", () => {
        ...
        expect(consoleInfoSpy).toHaveBeenCalledWith(expect.stringContaining("OpenTelemetry started successfully."));
    });

    it("logs error if OpenTelemetry fails to start", () => {
        const sdkMocks = require("@opentelemetry/sdk-node").mocks;
        sdkMocks.start.mockImplementationOnce(() => {
    ...
    it("logs when OpenTelemetry is disabled", () => {
        ...
        require("../../src/openTelemetry");
        expect(consoleInfoSpy).toHaveBeenCalledWith(expect.stringContaining("OpenTelemetry is disabled."));
    });
    ...
    it("shuts down OpenTelemetry if started", async () => {
        ...
        expect(consoleInfoSpy).toHaveBeenCalledWith(expect.stringContaining("OpenTelemetry shutdown complete."));
    });
