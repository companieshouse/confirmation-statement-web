...
const EXPECTED_ERROR_MESSAGE_LONG: string =
    "Radio value: very malicious code block that is extremely long a doesn't match the valid radio values";
...
describe("radio button validator tests", () => {
    ...
    describe("getRadioButtonInvalidValueErrorMessage tests", () => {
        ...
        it("Should return no more than 50 characters for the truncated radio value in the error message", () => {
            expect(getRadioButtonInvalidValueErrorMessage(INVALID_RADIO_VALUE_LONG)).toEqual(
                EXPECTED_ERROR_MESSAGE_LONG
            );
        });

        it("Should return undefined in the error message when radio value is undefined", () => {
            expect(getRadioButtonInvalidValueErrorMessage(undefined as unknown as string)).toEqual(
                EXPECTED_ERROR_MESSAGE_NULL
            );
        });
    });
});
