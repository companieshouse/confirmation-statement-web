...
import { OFFICER_DETAILS_ERROR, OFFICER_ROLE, RADIO_BUTTON_VALUE, SECTIONS } from "../../utils/constants";
import {
    equalsIgnoreCase,
    formatAddress,
...
export const get = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const officers: ActiveOfficerDetails[] = await getActiveOfficersDetailsData(
            session,
            transactionId,
            submissionId
        );
        ...
        .map(officer => {
            return {
                ...
                identificationType: officer.identificationType
                    ? lookupIdentificationType(officer.identificationType)
                    : "",
