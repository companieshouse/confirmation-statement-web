...
import { getActiveOfficerDetailsData, formatOfficerDetails } from "../../services/active.officer.details.service";

export const get = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const companyNumber = urlUtils.getCompanyNumberFromRequestParams(req);
    const session: Session = req.session as Session;
    const officerDetails: ActiveOfficerDetails = await getActiveOfficerDetailsData(session, companyNumber);
