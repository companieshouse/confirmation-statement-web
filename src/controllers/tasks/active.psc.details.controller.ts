...
import { PSC_STATEMENT_PATH, TASK_LIST_PATH, URL_QUERY_PARAM, WRONG_PSC_DETAILS_PATH } from "../../types/page.urls";
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (
            activePscsButtonValue === RADIO_BUTTON_VALUE.YES ||
            activePscsButtonValue === RADIO_BUTTON_VALUE.RECENTLY_FILED
        ) {
            ...
            const transactionId = urlUtils.getTransactionIdFromRequestParams(req);
            const submissionId = urlUtils.getSubmissionIdFromRequestParams(req);
            const pscs: PersonOfSignificantControl[] = await getPscs(
...
const buildIndividualPscList = (pscs: PersonOfSignificantControl[]): any[] => {
    return pscs
        ...
        .map(psc => {
            ...
            const serviceAddress = formattedPsc.serviceAddress
                ? formatAddressForDisplay(formattedPsc.serviceAddress)
                : "";
