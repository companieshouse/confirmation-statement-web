    ...
    if (companyProfile?.confirmationStatement?.nextMadeUpTo) {
        ...
        if (nextMadeUpToDate.isDue === false) {
            pageOptions["notDueWarning"] = {
                newNextMadeUptoDate: nextMadeUpToDate.newNextMadeUpToDate
                    ? toReadableFormat(nextMadeUpToDate.newNextMadeUpToDate)
                    : "",
            };
        }
    }
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!isCompanyValidForService(eligibilityStatusCode)) {
            ...
            return displayEligibilityStopPage(
                res,
                EligibilityStatusCode.INVALID_COMPANY_TYPE_PAPER_FILING_ONLY,
                company
            );
        }
...
const displayEligibilityStopPage = (
    res: Response,
