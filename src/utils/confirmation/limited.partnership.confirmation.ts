...
import {
    ...
    LP_LAWFUL_ACTIVITY_STATEMENT_ERROR,
...
export function handleLimitedPartnershipConfirmationJourney(
    ...
    const confirmationValid = isCheckboxTicked(confirmationCheckboxValue);

    req.session?.setExtraData(CONFIRMATION_STATEMENT_SESSION_KEY, confirmationValid ? "true" : "false");

    const lawfulActivityValid = isCheckboxTicked(lawfulActivityCheckboxValue);
...
export async function sendLimitedPartnershipTransactionUpdate(
    req: Request,
    newCsDate: string | null,
    newSicCodes: SicCodeData | null
) {
    const transactionId = urlUtils.getTransactionIdFromRequestParams(req);
    const submissionId = urlUtils.getSubmissionIdFromRequestParams(req);
    const session = req.session as Session;

    const currentCsSubmission: ConfirmationStatementSubmission = await getConfirmationStatement(
        session,
        transactionId,
        submissionId
    );
