...
export const executePaymentJourney = async (
    ...
    try {
        paymentUrl = await closeTransaction(session, companyNumber, submissionId, transactionId);
    } catch (err) {
        return next(err);
    }
    ...
    if (!paymentUrl) {
        ...
        const paymentResponse: ApiResponse<Payment> = await startPaymentsSession(
            session,
            paymentUrl,
            paymentResourceUri,
            submissionId,
...
export const isPaymentDue = (transaction: Transaction, submissionId: string): boolean => {
    ...
    const resourceKeyName = Object.keys(transaction.resources).find(key =>
        key.endsWith(`${CONFIRMATION_STATEMENT}/${submissionId}`)
    );
    if (!resourceKeyName) {
        return false;
    }
