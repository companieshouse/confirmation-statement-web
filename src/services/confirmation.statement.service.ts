...
export const createConfirmationStatement = async (
    session: Session,
    transactionId: string
): Promise<Resource<ConfirmationStatementCreated | CompanyValidationResponse>> => {
    ...
    if (response.httpStatusCode !== 201 && response.httpStatusCode !== 400) {
        ...
        throw new Error(
            `Something went wrong creating confirmation statement, transactionId = ${transactionId} - ${JSON.stringify(castedResponse)}`
        );
    } else {
...
export const getConfirmationStatement = async (
    session: Session,
    transactionId: string,
    confirmationStatementId: string
): Promise<ConfirmationStatementSubmission> => {
    const client = createPublicOAuthApiClient(session);
    const csService: ConfirmationStatementService = client.confirmationStatementService;
    ...
    if (response.httpStatusCode !== 200) {
        throw new Error(
            `Error getting confirmation statement from api with confirmationStatementId = ${encodedConfirmationStatementId}, transactionId = ${encodedTransactionId} - ${JSON.stringify(response)}`
        );
    }
    ...
    if (!csSubmissionResource.resource) {
        throw new Error(
            `Error No resource returned when getting confirmation statement from api with confirmationStatementId = ${encodedConfirmationStatementId}, transactionId = ${encodedTransactionId}`
        );
    }
...
export const updateConfirmationStatement = async (
    session: Session,
    transactionId: string,
    submitId: string,
    ...
    if (response.httpStatusCode !== 200) {
        const castedResponse: ApiErrorResponse = response;
        throw new Error(
            `Transaction Id ${encodedTransactionId}, Submit Id ${encodedSubmitId}, Something went wrong updating confirmation statement ${JSON.stringify(castedResponse)}`
        );
    }
};
...
export const getNextMadeUpToDate = async (session: Session, companyNumber: string): Promise<NextMadeUpToDate> => {
    ...
    if (response.httpStatusCode !== 200) {
        throw createAndLogError(
            `Error getting next made up to date from api with company number = ${companyNumber} - ${JSON.stringify(response)}`
        );
    }
    ...
    if (!nextMadeUpToDate.resource) {
        throw createAndLogError(
            `Error No resource returned when getting next made up to date from api with companyNumber = ${companyNumber} - ${JSON.stringify(response)}`
        );
    }
