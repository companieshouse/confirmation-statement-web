...
export const getConfirmationStatement = async (session: Session, transactionId: string, confirmationStatementId: string): Promise<ConfirmationStatementSubmission> => {
  ...
  if (response.httpStatusCode !== 200) {
    throw new Error(`Error getting confirmation statement from api with id = ${confirmationStatementId}, transactionID = ${transactionId} - ${JSON.stringify(response)}`);
  }

  const csSubmissionResource = response as Resource<ConfirmationStatementSubmission>;
  if (!csSubmissionResource.resource) {
    throw new Error(`Error No resource returned when getting confirmation statement from api with id = ${confirmationStatementId}, transactionID = ${transactionId}`);
  }
