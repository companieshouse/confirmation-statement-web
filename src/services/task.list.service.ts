...
import { TaskList, TaskState } from "../types/task.list";
...
export const initTaskList = (
    ...
    companyHasExistingRea: boolean
): TaskList => {
    const allTasks = {
        officers: {
            state: toTaskState(csSubmission.data?.activeOfficerDetailsData?.sectionStatus),
            url: urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
                officerSection(),
                companyNumber,
                transactionId,
                submissionId
        ...
        registeredEmailAddress: registeredEmailAddressOptionEnabled
            ? {
                  ...
                  url: urlUtils.getUrlWithCompanyNumberTransactionIdAndSubmissionId(
                      getRegisteredEmailAddressSectionUrl(companyHasExistingRea),
                      companyNumber,
                      transactionId,
                      submissionId
                  ),
    ...
    const completedTasks = Object.keys(allTasks).filter(
        key => allTasks[key] && allTasks[key].state === TaskState.CHECKED
    ).length;
