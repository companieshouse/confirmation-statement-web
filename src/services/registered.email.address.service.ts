...
export const getRegisteredEmailAddress = async (companyNumber: string): Promise<string> => {
    ...
    const response: Resource<RegisteredEmailAddressResponse> | ApiErrorResponse =
        await csService.getRegisteredEmailAddress(companyNumber);
    if (!response) {
    ...
    if (status !== 200) {
        ...
        if (
            ...
            errorResponse.errors[0].error === "Registered Email Address not found"
        ) {
        ...
        throw createAndLogError(
            `Error retrieving registered email address from confirmation-statement api for company number ${companyNumber}: ${JSON.stringify(response)}`
        );
    }
    const successfulResponse = response as Resource<RegisteredEmailAddressResponse>;
    if (!successfulResponse.resource) {
        throw createAndLogError(
            `No resource in response for registered email address from confirmation-statement api for company number ${companyNumber}: : ${JSON.stringify(successfulResponse)}`
        );
    }
