...
export const getActiveOfficersDetailsData = async (
    session: Session,
    transactionId: string,
    submissionId: string
): Promise<ActiveOfficerDetails[]> => {
    const client = createPublicOAuthApiClient(session);
    const csService: ConfirmationStatementService = client.confirmationStatementService;
    const response: Resource<ActiveOfficerDetails[]> | ApiErrorResponse = await csService.getListActiveOfficerDetails(
        transactionId,
        submissionId
    );
