...
export const getCompanyProfile = async (companyNumber: string): Promise<CompanyProfile> => {
    ...
    if (sdkResponse.httpStatusCode >= 400) {
        throw createAndLogError(
            `Http status code ${sdkResponse.httpStatusCode} - Failed to get company profile for company number ${companyNumber}`
        );
    }
...
export const formatForDisplay = (companyProfile: CompanyProfile): CompanyProfile => {
    ...
    if (companyProfile.confirmationStatement) {
        ...
        confirmationStatement.nextMadeUpTo = confirmationStatement.nextMadeUpTo
            ? toReadableFormat(confirmationStatement.nextMadeUpTo)
            : "";
    }
