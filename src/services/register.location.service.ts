...
export const getRegisterLocationData = async (
    session: Session,
    ...
    const response: Resource<RegisterLocation[]> | ApiErrorResponse = await csService.getRegisterLocations(
        transactionId,
        submissionId
    );
    ...
    if (status >= 400) {
        ...
        throw createAndLogError(
            `Error retrieving register location data from confirmation-statment api: ${JSON.stringify(errorResponse)}`
        );
    }
