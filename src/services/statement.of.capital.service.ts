...
export const getStatementOfCapitalData = async (
    session: Session,
    transactionId: string,
    submissionId: string
): Promise<StatementOfCapital> => {
    const client = createPublicOAuthApiClient(session);
    const csService: ConfirmationStatementService = client.confirmationStatementService;
    const response: Resource<StatementOfCapital> | ApiErrorResponse = await csService.getStatementOfCapital(
        transactionId,
        submissionId
    );
...
export const validateTotalNumberOfShares = async (
    session: Session,
    transactionId: string,
    submissionId: string,
    ...
    shareholders.forEach(
        shareholder => (shareholderTotalNumberOfShares = +shareholder.shares + shareholderTotalNumberOfShares)
    );
